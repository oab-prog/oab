...
export default function Index() {
  ...
  const handleClearData = async () => {
    if (confirm("Deseja realmente limpar seu progresso do simulado? Esta ação não pode ser desfeita.")) {
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user) {
        await (supabase.from("user_progress" as any) as any)
          .update({ simulado_data: null, historico_data: [] })
    ...
    <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-10">
      ...
      <div ref={reveal.ref} style={reveal.style} className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <Button 
          ...
          className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground hover:text-destructive flex items-center gap-1.5"
        >
          <RotateCcw className="h-3 w-3" /> Limpar Dados
        </Button>
      </div>
