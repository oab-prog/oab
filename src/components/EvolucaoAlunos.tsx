...
export default function EvolucaoAlunos() {
  const { history, loading } = useUserProgress();
  const [historico, setHistorico] = useState<any[]>([]);

  useEffect(() => {
    if (history) {
      setHistorico([...history].reverse());
    }
  }, [history]);

  if (loading) {
    return (
      <Card className="bg-secondary/10 border-white/5 h-[300px] flex items-center justify-center">
